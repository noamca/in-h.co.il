	...
	$('.ajax_load_asset').on('click',function(){
		...
		$.post(
			...
			function(data){
				...
				//modify name function, classes, ids for mobile version of the gallery in order to eliminate confilcts with desctop version.
				var mobile_data = data.replace( /galleryModal/g, 'galleryModalMobile' );
				mobile_data = mobile_data.replace( /modal-gallery-slider/g, 'modal-gallery-slider-mobile' );
				mobile_data = mobile_data.replace( 'modal-lg', 'modal-dialog-mobile' );
				mobile_data = mobile_data.replace( /openModal/g, 'openModalMobile' );
				
				div1.html(mobile_data);
				div2.html(data);				
				heightToAuto(div1);
				heightToAuto(div2);
